...
namespace VirtoCommerce.Storefront.Tests.Scriban
{
    public class ScribanTests
    {
        ...
        {
            const string template = "{% capture tag_label_template %}tags.{{ tag.group_type }}.{% if tag.lower and tag.upper %}between{% elsif tag.lower %}greater{% elsif tag.upper %}less{% endif %}{% endcapture %} {{ tag_label_template }}";
            var parsedTemplate = Template.ParseLiquid(template);
            Assert.False(parsedTemplate.HasErrors);

            var scriptObject = new ScriptObject();

            scriptObject.SetValue("tag", new Tag
            {
                GroupType = "pricerange"
                , Lower = "11$"
            }, true);
        ...
        public void IndexAccess_For_List()
        {
            ...
            scriptObject.Import(new TestContext
            {
                Products = new MutablePagedList<Product>(new List<Product>
                {
                    new Product
                    {
                        Id = "headphones"
